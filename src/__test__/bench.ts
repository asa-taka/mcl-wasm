...
function PairingTest () {
  ...
  assert.strictEqual(ab.getStr(), 123 * 456)
...
function mulVecGeneric <T extends mcl.Common>(Cstr: { new(): T}, xVec: any[], yVec: any[]):T {
  let z: any = new Cstr()
...
function serializeSubTest <T extends mcl.Common>(Cstr: { new(): T }, x: T, newDeserializeHexStr: (s: string) => T) {
  const y = new Cstr()
